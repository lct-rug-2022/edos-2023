...
def train(
        name: str,
        parameters_config_path: str = None,
    ...
    parameters_config: dict
        ...
        If not specified, config is read from `parameters_config_path`.
    dataset_config_paths: List[str]
        List of paths to dataset configurations.
    dataset_configs: List[dict]
        List of dataset configurations.
        If not specified, configs are read from `dataset_config_paths`.
    serialization_dir: str
        Path for model and output serialization.
    to_predict: bool
        Whether to run prediction at the end of training.
    device: str
    ...
    if resume:
        ...
        dataset_config_iterator = dataset_configs or dataset_config_paths
        dataset_configs = myutils.merge_configs(dataset_config_iterator, parameters_config)
    ...
    callback = Callback(
        ...
        keep_best_n=parameters_config['training']['keep_top_n']
    )
    ...
    for epoch in range(first_epoch, parameters_config['training']['num_epochs'] + 1):
        ...
        callback.end_epoch(epoch, model, neptune_run=neptune_run)
    ...
    if to_predict and len(dev_dataset) > 0:
