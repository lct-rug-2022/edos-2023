...
class Callback:
    ...
    def end_epoch(self, cur_epoch, model, neptune_run):
        ...
        self.report_scores(cur_epoch, best_epoch, neptune_run)
    ...
    def collect_score_info_for_epoch(self, epoch, epoch_name):
        info = {}
        table = [[epoch_name, '', '', '', '']]
    ...
    def log_neptune_task(self, neptune_run, scores, epoch, part, item):
        task_name = item[:item.rfind('-')] if '-' in item else item
        neptune_run[f'{part}/score/{item}'].log(scores['score'][epoch][item], step=epoch+1)
        neptune_run[f'{part}/loss/{item}'].log(scores['loss'][epoch][task_name], step=epoch+1)

    def log_neptune_epoch(self, epoch, neptune_run):
        keys = {
            'dev': {'loss': self.dev_losses, 'score': self.dev_scores},
            'train': {'loss': self.train_losses, 'score': self.train_scores}
        }

        for part, part_evals in keys.items():
            self.log_neptune_task(neptune_run, part_evals, epoch, part, 'sum')
            for item in sorted(part_evals['score'][0]):
                if item != 'sum':
                    self.log_neptune_task(neptune_run, part_evals, epoch, part, item)

    def report_scores(self, cur_epoch, best_epoch, neptune_run):
        ...
        best_epoch_info, best_epoch_table = self.collect_score_info_for_epoch(
            best_epoch, 'Best (' + str(best_epoch + 1) + ')'
        )
        table.extend(best_epoch_table)
        info.update(best_epoch_info)

        curr_epoch_info, curr_epoch_table = self.collect_score_info_for_epoch(
            cur_epoch, 'Epoch ' + str(cur_epoch + 1)
        )
        table.extend(curr_epoch_table)
        info.update(curr_epoch_info)

        if neptune_run is not None:
            self.log_neptune_epoch(cur_epoch, neptune_run)
